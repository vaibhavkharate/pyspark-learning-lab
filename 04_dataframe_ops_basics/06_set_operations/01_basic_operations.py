...
# Create DataFrames
# The above code initializes a Spark session and defines two sets of sample data, which are then converted into DataFrames. Each DataFrame consists of rows with three columns: 'id', 'name', and 'age'. The first DataFrame (df1) contains five records, while the second DataFrame (df2) contains five records, with some overlapping entries. These DataFrames will be used to demonstrate various set operations such as union, intersection, and difference.
...
# The union operation combines all rows from both DataFrames, including duplicates. This is useful when you want to merge datasets while retaining all entries, even if they are repeated. The resulting DataFrame will contain all records from df1 followed by all records from df2.
union_df = df1.union(df2)
union_df.show()
# SQL Equivalent
# The SQL equivalent of the union operation is performed using the UNION ALL clause, which combines the results of two SELECT statements. This allows you to retrieve all records from both tables, including duplicates, in a single result set.
...
# The intersect operation identifies rows that are common to both DataFrames. This is particularly useful for finding overlapping data between two datasets. The resulting DataFrame will contain only those records that exist in both df1 and df2.
