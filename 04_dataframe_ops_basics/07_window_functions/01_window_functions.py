...
# This allows us to run SQL queries against the DataFrame using Spark SQL.
...
# Using PySpark's dense_rank function to assign ranks to employees based on their salary within each department without gaps in ranking.
# This is useful for scenarios where you want to ensure that ranks are consecutive even if there are ties in salary.
...
# Using PySpark's row_number function to assign a unique row number to each employee based on their salary within their department.
# This is useful for identifying the order of employees within each department based on their salary.
...
# Using PySpark's avg function to compute a moving average of salaries within each department.
# This is useful for analyzing trends in salary changes over time within departments.
...
# Using PySpark's max function to determine the maximum salary up to each row within department.
# This is useful for identifying the highest salary earned by employees in a department up to a certain
...
# Using PySpark's min function to determine the minimum salary up to each row within department.
# This is useful for identifying the lowest salary earned by employees in a department up to a certain
...
# Using PySpark's lead function to find the next row's salary within the same department.
# This is useful for comparing an employee's salary with the next employee's salary in the same
...
# Using PySpark's lag function to find the previous row's salary within the same department.
# This is useful for comparing an employee's salary with the previous employee's salary in the same
...
# Using PySpark's lag function to find the previous row's salary within the same department.
# This is useful for calculating the percentage change in salary from the previous employee's salary in the
...
# Using PySpark's count function to calculate the cumulative count of employees within each department.
...
# Using PySpark's lead function to find the next row's sales within the same department.
# This is useful for comparing an employee's sales with the next employee's sales in the same department.
